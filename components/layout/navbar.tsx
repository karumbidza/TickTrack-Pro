        ...
        <div className="flex items-center gap-3">
          ...
          <div className="hidden lg:flex items-center gap-2">
            <Badge variant="neutral">
              {tenantName || (role === 'SUPER_ADMIN' ? 'Super Admin' : 'TickTrack Pro')}
            </Badge>
            {branchName && role !== 'SUPER_ADMIN' && (
              <Badge variant="info">
                {branchName}
              </Badge>
            )}
              ...
              <DropdownMenuContent className="w-56" align="end" forceMount>
                <div className="flex items-center gap-3 p-3">
                  ...
                  <div className="flex flex-col min-w-0">
                    {user?.fullName && (
                      <p className="text-sm font-medium truncate" style={{ color: 'var(--text-primary)' }}>{user.fullName}</p>
                    )}
                    {user?.primaryEmailAddress?.emailAddress && (
                      <p className="text-xs truncate" style={{ color: 'var(--text-muted)' }}>{user.primaryEmailAddress.emailAddress}</p>
                    )}
                  </div>
                </div>
                <DropdownMenuSeparator />
                <DropdownMenuItem asChild>
                  <Link href={role === 'TENANT_ADMIN' ? '/admin/settings' : '/dashboard'} className="cursor-pointer">
      ...
      {mobileMenuOpen && (
        <div className="md:hidden px-4 pb-4" style={{ borderTop: '1px solid var(--border)' }}>
          ...
          <div className="mt-3 pt-3 space-y-1" style={{ borderTop: '1px solid var(--border)' }}>
            <Link
              href={role === 'TENANT_ADMIN' ? '/admin/settings' : '/dashboard'}
              onClick={() => setMobileMenuOpen(false)}
